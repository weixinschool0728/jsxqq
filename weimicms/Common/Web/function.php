		...
		if(empty($pc_flash_cat)){
			if($attr['default_img']){
				for($i=0;$i<$attr['default_img'];$i++){
					$flash_list[$i] = array(
										'name' => '演示图片-'.($i+1),
										'pic'  => '{weimicms:WEB_STATIC_URL}images/banner'.($i+1).'.jpg',
									);
				}
			}
			return $flash_list;
...
function web_productcat_list($attr=array()){
	...
	if($productcat_list){
		foreach($productcat_list as $key=>$value){
			$productcat_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/productcat/'.$value['cat_id'].'.html';
		}
	}
	if($attr['limit']){
...
function web_product_list($attr=array()){
	...
	if(isset($attr['key'])){
		...
		$pc_product_category['cat_info']['url'] = '{weimicms:WEB_VISIT_URL}/productcat/'.$pc_product_category['cat_id'].'.html';
		...
		if($product_list['product_list']){
			foreach($product_list['product_list'] as $key=>$value){
				$product_list['product_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$value['id'].'.html';
			}
		}
		...
		if($product_list['product_list']){
			foreach($product_list['product_list'] as $key=>$value){
				$product_list['product_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$value['id'].'.html';
				$product_list['product_list'][$key]['cat_url'] = '{weimicms:WEB_VISIT_URL}/productcat/'.$value['cat_id'].'.html';
			}
		}
	...
	if(!empty($product_list)){
		foreach($product_list as $key=>$value){
			$product_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$value['id'].'.html';
		}
		if($attr['limit']){
...
function add_product_rows($product_list,$limit){
	...
	for($i=$count;$i<$limit;$i++){
		$product = array(
						'title'=>'演示产品-'.($i+1),
						'pic'=>'{weimicms:WEB_STATIC_URL}images/pro'.($i+1).'.jpg',
...
function web_downcat_list($attr=array()){
	...
	foreach($downcat_list as $key=>$value){
		$downcat_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/downcat/'.$value['cat_id'].'.html';
	}
...
function web_down_list($attr=array()){
	...
	if(isset($attr['key'])){
		...
		$pc_down_category['cat_info']['url'] = '{weimicms:WEB_VISIT_URL}/downcat/'.$pc_down_category['cat_id'].'.html';
		...
		if($down_list['down_list']){
			foreach($down_list['down_list'] as $key=>$value){
				$down_list['down_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$value['id'].'.html';
			}
		}
		...
		if($down_list['down_list']){
			foreach($down_list['down_list'] as $key=>$value){
				$down_list['down_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/down/'.$value['id'].'.html';
				$down_list['down_list'][$key]['cat_url'] = '{weimicms:WEB_VISIT_URL}/downcat/'.$value['cat_id'].'.html';
			}
		}
	...
	if(!empty($down_list)){
		foreach($down_list as $key=>$value){
			$down_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$value['id'].'.html';
		}
		if($attr['limit']){
...
function web_newscat_list($attr=array()){
	...
	foreach($newscat_list as $key=>$value){
		$newscat_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/newscat/'.$value['cat_id'].'.html';
	}
...
function web_news_list($attr=array()){
	...
	if(isset($attr['key'])){
		...
		$pc_news_category['cat_info']['url'] = '{weimicms:WEB_VISIT_URL}/newscat/'.$pc_news_category['cat_id'].'.html';
		...
		if($news_list['news_list']){
			foreach($news_list['news_list'] as $key=>$value){
				$news_list['news_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$value['id'].'.html';
			}
		}
		if($attr['limit']){
		...
		if($news_list['news_list']){
			foreach($news_list['news_list'] as $key=>$value){
				$news_list['news_list'][$key]['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$value['id'].'.html';
				$news_list['news_list'][$key]['cat_url'] = '{weimicms:WEB_VISIT_URL}/newscat/'.$value['cat_id'].'.html';
			}
		}
	...
	if(!empty($news_list)){
		foreach($news_list as $key=>$value){
			$news_list[$key]['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$value['id'].'.html';
		}
		if($attr['limit']){
...
function add_news_rows($news_list,$limit){
	...
	for($i=$count;$i<$limit;$i++){
		$news = array(
						'title'=>'演示文章-'.($i+1),
						'pic'=>'{weimicms:WEB_STATIC_URL}images/news_default.jpg',
...
function get_page($type,$cat_id,$total,$now){
	$url = '{weimicms:WEB_VISIT_URL}/'.$type.'/'.$cat_id.'-';
	$str = '<div class="list">';
	if ($now >= 2){
...
function pagelink($key){
	echo '{weimicms:WEB_VISIT_URL}/page/'.$key.'.html';
} 
...
function web_one_page($attr=array()){
	...
	$one_page['url'] = '{weimicms:WEB_VISIT_URL}/page/'.$one_page['key'].'.html';
	return $one_page;
}
