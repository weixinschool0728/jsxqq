...
class RecognitionDataAction extends UserAction
{
	...
	'wechaname' => array('like', '%' . $_GET['name'] . '%')
		...
		if ($is_info == 'y') {
			...
			$data = M('recognition_data')->where($where)->order('tp_id desc')->limit($page->firstRow . ',' . $page->listRows)->select();
		}
		else {
			$sdata = M('recognition_data')->where($where)->order('tp_id desc')->select();
		...
		if ($_GET['rid'] != '') {
			...
			$data111 = M('recognition_data')->where($where)->order('tp_id')->select();
