...
// 		}
	...
	//企秀同步
	public function eqx(){
		        $users=M('Users')->where(array('id'=>session('uid')))->find();//获得当前用户的密码写入微赞秀
				...
				if ($b){
				  ...
				  $a=$db->save($date);
				  if($a){$this->success('帐号同步成功',U(MODULE_NAME.'/index'));}else{$this->error('帐号同步失败,你没有权限使用微赞秀，请联系管理员',U(MODULE_NAME.'/index'));}
				}else{
                  $this->error('帐号同步失败,你没有权限使用微赞秀，请联系管理员',U(MODULE_NAME.'/index'));
	...
	function bomb_ajax(){
		...
		if(!empty($_POST['set_id'])){
			...
			if($user_arr['winxintype']=='3' && C('server_topdomain') == 'weimicms.cn' && empty($_COOKIE[$token.'set_id'])){
