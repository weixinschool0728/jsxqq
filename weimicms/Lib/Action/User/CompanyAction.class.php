...
class CompanyAction extends UserAction{
	...
	public function updateallpoi(){
		$company 	= $this->company_model->where(array('token'=>$this->token,'available_state'=>array('neq',3)))->select();
		...
		foreach($business_list as $key=>$val){
			if($val['base_info']['sid'] != ''){
				$this->company_model->where(array('token'=>$this->token,'sid'=>$val['base_info']['sid']))->save(array('location_id'=>$val['base_info']['poi_id'],'available_state'=>$val['base_info']['available_state']));
			}
		}
