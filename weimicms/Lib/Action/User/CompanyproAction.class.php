...
class CompanyproAction extends UserAction
{
	public $token;
	public $isBranch;
	...
	public function _initialize()
	{
		...
		$this->company_model = m('Companypro');
	}
	...
	public function index()
	{
		...
		if (IS_POST) {
			...
			if (!$thisCompany) {
				if ($this->isBranch) {
					$this->insert('Companypro', u('Companypro/branches', array('token' => $this->token, 'isBranch' => $this->isBranch)));
				}
				else {
					$this->insert('Companypro', u('Companypro/index', array('token' => $this->token, 'isBranch' => $this->isBranch)));
				}
			}
			else {
				$amap = new amap();
				...
				if ($this->company_model->create()) {
					...
					if ($this->company_model->where($where)->save($_POST)) {
						if ($this->isBranch) {
							$this->success('修改成功', u('Companypro/branches', array('token' => $this->token, 'isBranch' => $this->isBranch)));
						}
						else {
							$this->success('修改成功', u('Companypro/index', array('token' => $this->token, 'isBranch' => $this->isBranch)));
						}
					}
					else {
						$this->error('操作失败');
	...
	public function delete()
	{
		...
		if ($rt == true) {
			...
			$this->success('删除成功', u('Companypro/branches', array('token' => $this->token, 'isBranch' => 1)));
		}
		else {
			$this->error('服务器繁忙,请稍后再试', u('Companypro/branches', array('token' => $this->token, 'isBranch' => 1)));
		}
	}
}
