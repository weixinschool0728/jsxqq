<?php
class MsAction extends UserAction{
	public $token;
	...
	public function products(){		
		$where=array('token'=>$this->token,'ms'=>1);
        if(IS_POST){
		...
		if(IS_POST){ 
			...
			if($this->product_model->create()){
				...
				if($this->product_model->where($where)->save($_POST)){
					$this->success('修改成功',U('Ms/products',array('token'=>$this->token)));
	...
	public function del(){
        ...
        if(IS_GET){                              
            ...
            if($back==true){
                ...
                $this->success('操作成功',U('Ms/products',array('token'=>$this->token)));
            }else{
                 $this->error('服务器繁忙,请稍后再试',U('Ms/products',array('token'=>$this->token)));
            }
        }        
	}
	public function index(){
		if (IS_POST){
			...
			$this->success('操作成功',U('Ms/index',array('token'=>session('token'),'dining'=>$this->isDining)));
		}else{
			$where=array('token'=>$this->token,'ms'=>1);
			if(IS_POST){
				$key = $this->_post('searchkey');
			...
			$unHandledCount=$this->product_cart_model->where(array('token'=>$this->token,'handled'=>0,'Ms'=>1))->count();
	...
	public function config(){
		...
		S('MsConfig'.$this->token,$thisInfo);
		...
		if(IS_POST){
			...
			if ($thisInfo){
				...
				$this->success('修改成功',U('Ms/config',$where));
						
			}else {
				$where=array('infotype'=>$thisInfo['infotype'],'token'=>$this->token);
				$this->reply_info_model->add($row);
				$this->success('设置成功',U('Ms/config',$where));
			}
		}else{
			//
