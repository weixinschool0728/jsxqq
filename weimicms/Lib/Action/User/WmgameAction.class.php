<?php
class WmgameAction extends UserAction{
		public $token;
	...
	public function index(){
		 $this->reply_info_model=M('Wmgamereply_info');
		...
		if(IS_POST){
			...
			if ($thisInfo){
				...
				$this->success('修改成功',U('Wmgame/index',$where));
						
			}else {
				$where=array('token'=>$this->token);
				$this->reply_info_model->add($row);
				$this->success('设置成功',U('Wmgame/index',$where));
			}
		}else{
			//
