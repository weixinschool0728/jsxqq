<?php
class YXDiymenAction extends UserAction{
	//自定义菜单配置
	...
	public function  class_send(){
		if(IS_GET){
			...
			//dump($api);
			/***
			$url_get='https://api.yixin.im/cgi-bin/token?grant_type=client_credential&appid='.$api['appid'].'&secret='.$api['appsecret'];
			$json=json_decode($this->curlGet($url_get));
  	if($api['appid']==false||$api['appsecret']==false){$this->error('必须先填写【AppId】【 AppSecret】');exit;}
   $data = '{ "button" : [ ';
			***/
			$url_get='https://api.yixin.im/cgi-bin/token?grant_type=client_credential&appid='.$api['yxappid'].'&secret='.$api['yxappsecret'];
			$json=json_decode($this->curlGet($url_get));
  	if($api['yxappid']==false||$api['yxappsecret']==false){$this->error('必须先填写【AppId】【 AppSecret】');exit;}
   $data = '{ "button" : [ ';
			$class=M('Diymen_class')->where(array('token'=>session('token'),'pid'=>0,'is_show'=>1))->limit(4)->order('sort desc')->select();//dump($class);
			$kcount=M('Diymen_class')->where(array('token'=>session('token'),'pid'=>0,'is_show'=>1))->limit(4)->order('sort desc')->count();
			$k=1;
			foreach($class as $key=>$vo){
...
//dump($json->access_token);
			...
			$rt=$this->curl_file_get_contents($url,$data);
			if($rt['rt']==false){
