<?php
class GamettAction extends UserAction{
		public $token;
	
	public function _initialize() {
		parent::_initialize();
		$this->canUseFunction('Gamett');
		$this->token=session('token');
		$this->assign('token',$this->token);
	}
	public function index(){
		 $this->reply_info_model=M('Gamettreply_info');
		...
		if(IS_POST){
			...
			if ($thisInfo){
				...
				$this->success('修改成功',U('Gamett/index',$where));
						
			}else {
				$where=array('token'=>$this->token);
				$this->reply_info_model->add($row);
				$this->success('设置成功',U('Gamett/index',$where));
			}
		}else{
			//
