...
class Web_indexAction extends Action{
	...
	public $now_website;
	public $site_config;
	public $static_url = 'http://webstatic.weimicms.cn';
	...
	public function news(){
		...
		$now_news['hits'] += 1;
		$now_news['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$now_news['id'].'.html';
		...
		$now_category['url'] = '{weimicms:WEB_VISIT_URL}/newscat/'.$now_category['cat_id'].'.html';
		$this->assign('now_category',$now_category);
		$this->assign('page_key',$now_category['cat_key']);
		...
		if(!empty($prev_news)){
			$prev_news['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$prev_news['id'].'.html';
		}
		...
		if(!empty($next_news)){
			$next_news['url'] = '{weimicms:WEB_VISIT_URL}/news/'.$next_news['id'].'.html';
		}
	...
	public function productcat(){
		...
		if(!empty($cat_id)){
			...
			$now_category['url'] = '{weimicms:WEB_VISIT_URL}/productcat/'.$now_category['cat_id'].'.html';
			$this->assign('now_category',$now_category);
	...
	public function product(){
		...
		$now_product['hits'] += 1;
		$now_product['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$now_product['id'].'.html';
		...
		if(!empty($prev_product)){
			$prev_product['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$prev_product['id'].'.html';
		}
		...
		if(!empty($next_product)){
			$next_news['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$next_product['id'].'.html';
		}
	...
	public function downcat(){
		...
		if(!empty($cat_id)){
			...
			$now_category['url'] = '{weimicms:WEB_VISIT_URL}/downcat/'.$now_category['cat_id'].'.html';
			$this->assign('now_category',$now_category);
	...
	public function down(){
		...
		$now_down['hits'] += 1;
		$now_down['url'] = '{weimicms:WEB_VISIT_URL}/product/'.$now_down['id'].'.html';
		...
		if(!empty($prev_down)){
			$prev_down['url'] = '{weimicms:WEB_VISIT_URL}/down/'.$prev_down['id'].'.html';
		}
		...
		if(!empty($next_down)){
			$next_news['url'] = '{weimicms:WEB_VISIT_URL}/down/'.$next_down['id'].'.html';
		}
