...
class UnitaryAction extends WapAction
{
		...
		if ($_GET['unitaryid'] != '') {
			...
			if ($unitary['state'] != 2) {
				...
				if ($pay_count < $unitary['price']) {
					...
					$model->sendTempMsg('TM00695', array('href' => $this->siteUrl . U('Wap/Unitary/goodswhere', array('token' => $this->token, 'unitaryid' => $_GET['unitaryid'])), 'wecha_id' => $find_lucknum2['wecha_id'], 'title' => '一元夺宝中奖通知', 'headinfo' => '恭喜您在一元夺宝中获得【' . $unitary['name'] . '】点击查看', 'program' => $unitary['name'], 'result' => date('Y年m月d日H时i分s秒'), 'remark' => ''));
				}
			}
		}
	}
	...
	public function cartajax()
	{
		...
		case 'delorder':
			...
			foreach ($order_list2 as $vo2) {
				...
				$where_cart2['order_id'] = $vo2['tp_id'];
		...
		case 'click_cart_count':
			...
			foreach ($cart_list as $vo) {
				...
				if (($lucknum_count < 1) && ($vo['addtime'] < (time() - (60 * 15)))) {
	...
	'addtime'  => array('gt', 1444663403)
		...
		if ($myorder != NULL) {
			$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $myorder['tp_id']));
		}
	}
	...
	public function payend()
	{
		if (IS_POST) {
			...
			$myorder = M('unitary_order')->where(array('token' => $_POST['token'], 'wecha_id' => $_POST['wecha_id'], 'tp_id' => $orderid))->find();

			if ($myorder['addtime'] != 0) {
				ini_set('memory_limit', '-1');
				$cart_list = $this->m_cart->where(array('token' => $_POST['token'], 'wecha_id' => $_POST['wecha_id'], 'state' => 1, 'order_id' => $orderid))->order('id')->select();
				...
				foreach ($cart_list as $key => $val) {
					...
					$lucknum_cart_count = M('unitary_lucknum')->where(array('token' => $_POST['token'], 'wecha_id' => $_POST['wecha_id'], 'order_id' => $orderid, 'cart_id' => $val['id'], 'unitary_id' => $val['unitary_id']))->count();
					...
					if (0 < intval($val['count_s'])) {
						...
						if (($unitary['price'] <= $pay_count) && ($unitary['state'] != 2)) {
							...
							$find_lucknum2 = $this->m_lucknum->where($where_lucknum2)->find();
							$model = new templateNews();
							$model->sendTempMsg('TM00695', array('href' => $this->siteUrl . U('Wap/Unitary/goodswhere', array('token' => $_POST['token'], 'unitaryid' => $val['unitary_id'])), 'wecha_id' => $find_lucknum2['wecha_id'], 'title' => '一元夺宝中奖通知', 'headinfo' => '恭喜您在一元夺宝中获得【' . $unitary['name'] . '】点击查看', 'program' => $unitary['name'], 'result' => date('Y年m月d日H时i分s秒'), 'remark' => ''));
						}

						$update_order = M('unitary_order')->where(array('token' => $_POST['token'], 'tp_id' => $orderid))->save(array('addtime' => 0));
					}
					else {
						$update_order = M('unitary_order')->where(array('token' => $_POST['token'], 'tp_id' => $orderid))->save(array('addtime' => 0));
					}
	...
	public function buyres()
	{
		...
		foreach ($cart_list as $k => $vo) {
			...
			if (($cart_list[$k]['count'] < 1) && ($vo['addtime'] < (time() - (60 * 15)))) {
				$this->m_cart->where(array('token' => $_POST['token'], 'id' => $vo['id']))->delete();
			}

			$cart_list[$k]['count'] = $cart_list[$k]['count'] ? $cart_list[$k]['count'] : $vo['count'] . '(未付款)';
		}
