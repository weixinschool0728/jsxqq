...
class CutpriceAction extends WapAction
{
	...
	public function dobuy()
	{
		...
		if ($order_is != '') {
			...
			if ($order_is2 != '') {
				...
				$update_order = $this->m_order->where(array('token' => $this->token, 'tp_id' => $id_order))->save(array('orderid' => $orderid));
	...
	{
		$where_order['token'] = $this->token;
		$where_order['tp_id'] = $_GET['orderid'] * 1;
