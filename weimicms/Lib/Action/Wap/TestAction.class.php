...
class TestAction extends WapAction{
	...
	public function index(){
		...
		$where_test['tp_id'] = (int)($_GET['id']);
		$test = $this->m_test->where($where_test)->find();
		$save['pv'] = $test['pv'] + 1;
		$update = $this->m_test->where($where_test)->save($save);
		$this->assign("test",$test);
		$where_test_list['token'] = $this->token;
		$test_list1 = $this->m_test->where($where_test_list)->order("tp_id desc")->select();
		$test_list2 = $this->m_test->where($where_test_list)->order("tp_id")->select();
		if($test_list1[0]['tp_id'] == (int)($_GET['id'])){
			$next['id'] = $test_list2[0]['tp_id'];
			...
			$where_test_next['tp_id'] = $test_list2[($test_next[(int)($_GET['id'])]+1)]['tp_id'];
			$find_test_next = $this->m_test->where($where_test_next)->find();
			$next['id'] = $find_test_next['tp_id'];
