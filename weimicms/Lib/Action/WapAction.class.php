...
class WapAction extends BaseAction
{
	...
	protected function _initialize()
	{
		...
		$toAuth = 0;
		if ((C('server_topdomain') == 'weimicms.cn') && (C('site_url') != 'http://demo2.weimicms.cn')) {
			$toAuth = 1;
		}
		else {
		...
		if ((C('server_topdomain') == 'weimicms.cn') && ($this->wxuser['winxintype'] != 3)) {
		...
		if (empty($_SESSION[$session_openid_name]) && (0 < $users['is_syn'])) {
			...
			$url = $users['source_domain'] . A('Home/Auth')->getCallbackUrl($users['is_syn'], 'auth') . 'token=' . $this->token . '&source=weimicms';
			header('Location:' . $url);
			exit();
		}
