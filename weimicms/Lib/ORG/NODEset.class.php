...
class NODEset{
	public function index(){
		...
		$genjiedian_Site = M('Node')->where(array('name'=>'Site','pid'=>intval($genjiedian),'level'=>0,'status'=>1,'display'=>1))->getField('id');
		$CheckUpdate = M('Node')->where(array('pid'=>intval($genjiedian_Site),'name'=>'Update','level'=>2,'status'=>1,'display'=>2))->find();
		$DoSqlUpdate = M('Node')->where(array('pid'=>intval($genjiedian_Site),'name'=>'DoSqlUpdate','level'=>2,'status'=>1,'display'=>2))->find();
		if($CheckUpdate == ''){
			$CheckUpdate = M('Node')->add(array('title'=>'更新程序','pid'=>intval($genjiedian_Site),'name'=>'Update','level'=>2,'status'=>1,'display'=>2));
		}
		if($DoSqlUpdate == ''){
