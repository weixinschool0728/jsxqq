...
class UserAction extends BaseAction{
	...
	protected function _initialize(){
		...
		/*if (C('server_topdomain')=='weimicms.cn'&&!C('close_user_notice')){
			$updateRecord=M('System_info')->order('lastsqlupdate DESC')->find();
			$updateUrl='http://up.weimicms.cn/oa/admin.php?m=notice&c=view&a=noticeByModule&group='.GROUP_NAME.'&module='.MODULE_NAME.'&fileid='.$updateRecord['version'];
	...
	protected function replaceUrl($url, $params = array()) {
		...
		if (!empty($url['path'])) { // /index.php 开头 或 index.php开头 侧替换
			if (empty($result)) {
				...
				$result .= $url['path']; // 当为 完整url时的处理  如 http://www.weimicms.com/index.php
			}
		} else {
			if (empty($result)) {
