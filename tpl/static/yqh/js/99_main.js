...
*/
	...
	scrollStop	: function(){
		...
		//报名表单滚动
		$('.wrapSige').on('touchstart',ylLightAPP14.windowScrollget);
		$('.wrapSige').on('touchmove',ylLightAPP14.scrollControl);
		$('.wrapSige').on('touchend',ylLightAPP14.windowScrollBack);
	},
	...
	scrollStart : function(){		
		...
		//报名表单滚动
		$('.wrapSige').off('touchstart');
		$('.wrapSige').off('touchmove');
		$('.wrapSige').off('touchend');
	},
	...
	scrollControl	: function(e){
		...
		if($(this).hasClass('speakCon') || $(this).hasClass('wrapSige')){
...
*/
	...
	sigeUpPopupOpen : function(){
		...
		$('.u-sigeUp').on('click',function(){
			...
			if(sigeUp.length >= 1) {
				if($('#check_img').val() != 'yes' && $('#members_limit').val() != 'yes'){
					//报名统计请求
 ...
 */
	...
	signUpCheck_submit	: function (form){
	 	loadingPageShow();
		var url = 'index.php?ac=yqh_bm&id='+id+'&yqh_id='+yqh_id;
		/**
		 *  ajax提交数据
		 */
		$.ajax({
			...
			success: function(msg){
				...
				if(msg.code=='200'){   
					...
					$('#check_img').val('yes');
					//提交成功，报名按钮变样式
					$('.u-sigeUp').addClass('success').find('a').text('已报名了');
				} else if(msg.code=='201') {
					$('.popup_sucess').html('已报名了');
					$(".popup_sucess").addClass("on");
					$(".popup_error").removeClass("on");
					$('#check_img').val('yes');
					//提交成功，报名按钮变样式
					...
					$('.popup_sucess').html('人数已满');
					$(".popup_sucess").addClass("on");
					$(".popup_error").removeClass("on");
					$('#members_limit').val('yes');
					//提交成功，报名按钮变样式
					$('.u-sigeUp').addClass('success').find('a').text('人数已满');
				} else if(msg.code=='401') {
					$('.popup_error').html('网络不稳定，请稍后重试！');
