...
function rotate() {
    ...
    var u = gs;
    var d = {
        home: u + "&a=info&token="+token+"&wecha_id="+wxid+"&id="+l,
        run: u + "&a=run&token="+token+"&wecha_id="+wxid+"&id="+l,
        submit: u + "&a=submit&token="+token+"&wecha_id="+wxid+"&id="+l,
        giveup: u + "&a=giveUpPrize&token="+token+"&wecha_id="+wxid+"&id="+l,
        exchange: u + "&a=setStatus&token="+token+"&wecha_id="+wxid+"&id="+l
    };
    function a() {
        if (o) {
            if (o.paused) {
        ...
        $.ajax({
            ...
            if (y.ret == 0) {
                ...
                if (A.myRecord) {
                    ...
                    x(z);
                    window.WBPage.GAME = {};
                    window.WBPage.GAME.name = A.myRecord.uname;
    ...
    function k(C) {
        ...
        if (A && $.isArray(A) && A.length > 0 && parseInt(C.advset.displaywinner) == 1) {
            ...
            for (var E in A) {
                y += "<li>";
                y += '恭喜 <span class="font-c63535">' + A[E].wecha_name + "</span> 摇中 " + A[E].prize;
                y += "</li>"
            }
            y += "</ul>";
    ...
    function s() {
        $.checkUID("请点击右上角菜单->查看公众号,关注后回复:"+keyword);
        $.ajax({
            type: "post",
            dataType: "json",
            url: d.run,
    ...
    function h(B, y) {
        ...
        if(z - 1==0){
        	A += '<div class="no-prize-box-btn">分享到朋友圈可获得摇奖次数哦</div>';
        }else{
