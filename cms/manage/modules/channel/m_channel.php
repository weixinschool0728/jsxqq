		...
		if (IS_POST){
			...
			if (!isset($_POST['id'])){//add
				...
				if ($info['homepicturechannel']){
					$this->channel_db->update(array('homepicturechannel'=>0),'token=\''.$info['token'].'\' AND id!='.$channelid);
				}
				if ($info['hometextchannel']){
					$this->channel_db->update(array('hometextchannel'=>0),'token=\''.$info['token'].'\' AND id!='.$channelid);
				}
				delCache('navChannels'.$this->siteid);
				//
				...
				//设置其他栏目为非首页显示
				if ($info['homepicturechannel']){
					$this->channel_db->update(array('homepicturechannel'=>0),'token=\''.$thisChannel['token'].'\' AND id!='.$_POST['id']);
				}
				if ($info['hometextchannel']){
					$this->channel_db->update(array('hometextchannel'=>0),'token=\''.$thisChannel['token'].'\' AND id!='.$_POST['id']);
				}
